...
import {
  createGeminiHandoffPrompt,
  type GeminiTutorMode,
} from "@/modules/ai/application/create-gemini-handoff";
import type {
  A1Curriculum,
  A1Day,
  A1LessonSection,
...
import {
  germanAlphabetFoundation,
  highValueSpellingPatterns,
} from "@/modules/curriculum/domain/german-pronunciation-foundation";
import {
  learnerText,
  notebookGuidance,
} from "@/modules/curriculum/domain/learner-content";
import {
  dailyCoreItems,
  lessonFlow,
  sectionsForFlow,
...
function ActiveSentenceExercise({
  ...
  const reset = () => {
    ...
    if (result !== "answer_revealed") {
      setResult(
        incorrectAttempts === 1 ? "strong_hint" : "idle",
      );
    }
  };
  ...
  const check = () => {
    if (
      normaliseSentence(builtSentence) ===
      normaliseSentence(exercise.answer)
    ) {
      setResult("complete");
      onComplete();
    } else {
...
function SentenceBuilder({
  ...
  const allActiveComplete =
    ...
    activeExercises.every(
      (exercise) => completedActive[exercise.id],
    );
  ...
  if (activeExercises.length === 0) {
    return (
      ...
      >
        ...
        <h3 id="sentence-builder-heading">
          Sentence Builder
        </h3>

        <p>
          This day contains guided or read-only
          sentence-building material.
        </p>

        {readOnlyExercises.map((exercise) => (
          <div
            key={exercise.id}
            className="read-only-exercise"
          >
            <div className="eyebrow">
  ...
  return (
    ...
    >
      ...
      <p>
        Build each target using the tokens provided.
      </p>
      ...
      {activeExercises.map((exercise) => (
        <ActiveSentenceExercise
          ...
          disabled={
            disabled ||
            completedActive[exercise.id]
          }
          onComplete={() =>
            handleActiveComplete(exercise.id)
          }
        />
      ))}
      ...
      {readOnlyExercises.length > 0 ? (
        <div className="read-only-exercises">
          <h4>
            Additional sentence-building material
          </h4>
...
function TutorHandoff({
  ...
  const [mode, setMode] =
    useState<GeminiTutorMode>("Guide Me");
  ...
  return (
    <section className="tutor-card" id="tutor">
      <div className="eyebrow">
        Gemini tutor handoff
      </div>

      <h3>
        Take today&apos;s exact context to your tutor
      </h3>

      <div
        className="handoff-tabs"
        aria-label="Tutor handoff type"
      >
      ...
      <div className="button-row">
        ...
        {copied ? (
          ...
          <span className="muted">
            No automatic AI connection is used.
          </span>
        )}
      </div>
    </section>
...
function LessonSection({
  ...
  section: A1LessonSection;
}) {
  return (
    <section
      className={`lesson-section ${
        section.kind === "daily_german_core"
          ? "daily-core"
          : ""
      }`}
      id={section.id}
    >
      ...
      {section.kind === "pronunciation" ? (
        <p className="audio-note">
          Audio not available yet. Use the approved
          learner-facing text guidance below; no audio
          is being simulated.
        </p>
      ) : null}
...
function TodayTargets({
  ...
  const remaining = [
    ...
    !progress.practiceCompleted
      ? "Confirm today’s practice"
      : null,
  ].filter(Boolean);

  return (
    <section
      className="today-targets"
      aria-labelledby="today-targets-heading"
    >
      <div className="section-kicker">
        Today&apos;s targets
      </div>
      ...
      <h2 id="today-targets-heading">
        A focused study plan from the approved Day{" "}
...
function DailyGermanCore({
  ...
  return (
    ...
    >
      <div>
        <div className="section-kicker">
          Separate reinforcement layer
        </div>

        <h2 id="daily-core-heading">
          Daily German Core
        </h2>

        <p>
          Lightweight current-day language for real
          contact. It is separate from the main lesson,
          not a second course.
        </p>
      </div>
      ...
      <p className="core-context">
        Practical context and approved examples appear
        in today&apos;s Real-life mission. The canonical
        day does not separately label every Core item
        active or receptive.
      </p>
    </section>
  );
...
function PronunciationFoundation({
  day,
}: {
  day: A1Day;
}) {
  ...
  return (
    <details
      className="pronunciation-foundation"
      id="alphabet-foundation"
    >
      <summary>
        <span>Pronunciation Foundation</span>

        <small>
          German alphabet and high-value spelling
          patterns · introduced in Day 5
        </small>
      </summary>

      <p className="audio-note">
        Audio not available yet. These are learner-facing
        spelling references, not simulated recordings.
      </p>

      <div className="alphabet-grid">
        {germanAlphabetFoundation.map(
          (entry) => (
      ...
      <p className="pronunciation-path">
        Use the progression already established in the
        curriculum: letter/sound → syllable → word →
        phrase → sentence → short connected speech.
      </p>
    </details>
  );
}
...
function LearningFlow({ day }: { day: A1Day }) {
  return (
    ...
    >
      ...
      <a href="#sentence-builder-heading">
        Build
      </a>
...
function FlowGroup({
  ...
  const sections = sectionsForFlow(
    day,
    kinds,
  );
    ...
    >
      ...
      {id === "learn" ? (
        <p className="flow-intro">
          Vocabulary and reusable patterns are kept in
          the approved Main lesson rather than duplicated
          into a second word list.
        </p>
      ) : null}
      ...
      {id === "pronunciation" ? (
        <PronunciationFoundation day={day} />
      ) : null}
    </section>
  );
}
...
function GuidedPracticeTasks({
  ...
  return (
    <section className="guided-practice">
      ...
      <p>
        These canonical prompts ask for active recall or
        a practical response. Mark completion only after
        you have done the task; no unsupported automatic
        score is claimed.
      </p>

      {tasks.map((task) => (
        <article
          className="guided-task"
          key={task.id}
        >
...
export function A1LearningMvp({
  ...
  const [progress, setProgress] =
    useState<LocalLearningProgress>(
      createLocalProgress,
    );
  ...
  const activeDay =
    curriculum.days.find(
      (day) =>
        day.dayNumber ===
        progress.currentDay,
    ) ?? curriculum.days[0];

  const activeProgress = dayProgress(
    progress,
    activeDay.dayNumber,
  );

  const activeNotebookNotes =
    notebookGuidance(activeDay);

  const nextLearningDay =
    curriculum.days.find((day) => {
      const item = dayProgress(
        progress,
        day.dayNumber,
      );
    ...
    <main className="app-shell">
      ...
      >
        <div>
          ...
          <h1>
            Day {nextLearningDay.dayNumber}:{" "}
            {nextLearningDay.title}
          </h1>

          <p>
            {learnerText(
              nextLearningDay.objective,
            )}
          </p>
        ...
        <aside className="dashboard-aside">
          ...
          <p>
            Review is learner-controlled in this local
            milestone. No SRS claim is made.
          </p>
          ...
          <a
            ...
            href="/pre-a1"
          >
            New to German? Try the optional Pre-A1
            foundation
          </a>
        </aside>
      </section>
      ...
      >
        ...
        <p className="muted">
          Only Days{" "}
          {curriculum.learningScope.firstDay}–
          {curriculum.learningScope.lastDay}{" "}
          are available in this learning milestone.
          Day{" "}
          {curriculum.learningScope.lastDay + 1}{" "}
          and later are not exposed.
        </p>
      </section>
      ...
      {view === "review" ? (
        <section className="review-panel">
          ...
          <h2>
            Return to a marked learning day
          </h2>
          ...
          {reviewDays.length ? (
            ...
            <p>
              Nothing is marked Needs Review. Mark any
              day from its lesson page when you want to
              revisit it.
            </p>
          )}
        </section>
        ...
        >
          ...
          {activeDay.finalQaNotices.length ? (
            <aside className="qa-notice">
              <strong>
                Approved final QA correction
              </strong>

              {activeDay.finalQaNotices.map(
                (notice) => (
                  <p key={notice}>
                    {learnerText(notice)}
                  </p>
                ...
                {flow.id === "learn" ? (
                  <section className="notebook-guidance">
                    ...
                    <div>
                      ...
                      <ul>
                        <li>
                          Temporary explanations and
                          source citations.
                        </li>
                        <li>
                          Repeated examples that do not
                          add a reusable pattern.
                        </li>
                        <li>
                          Exercise instructions you can
                          revisit here.
                        </li>
                      </ul>
                    </div>
          ...
          <section className="completion-card">
            ...
            <p>
              Lesson and practice are separate. Mastery
              status never controls whether the next day
              unlocks.
            </p>
          ...
          <details className="qa-overrides">
            <summary>
              Canonical final QA overrides for this day
            </summary>

            <MarkdownContent
              markdown={
                activeDay.finalQaOverrides
              }
            />
          </details>
        </article>
